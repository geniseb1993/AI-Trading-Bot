...
def receive_tradingview_webhook():
    ...
    try:
        ...
        print(f"Received webhook data: {alert_data}")
        
        if not alert_data:
            logger.warning("Received empty webhook data")
            print("Webhook ERROR: Empty data received")
            return jsonify({
                'success': False,
        ...
        if 'timestamp' not in alert_data and 'time' not in alert_data:
            alert_data['timestamp'] = datetime.now().isoformat()
        elif 'time' in alert_data and 'timestamp' not in alert_data:
            # Rename time to timestamp for consistency
            alert_data['timestamp'] = alert_data['time']
        
        # Convert 'signal' field to 'action' if present
        if 'signal' in alert_data and 'action' not in alert_data:
            alert_data['action'] = alert_data['signal']
        
        # Store the alert (limit to MAX_ALERTS)
        webhook_alerts.append(alert_data)
        print(f"Stored alert, total alerts: {len(webhook_alerts)}")
        
        if len(webhook_alerts) > MAX_ALERTS:
            webhook_alerts.pop(0)  # Remove oldest alert
            print(f"Removed oldest alert, total alerts: {len(webhook_alerts)}")
        
        logger.info(f"Received TradingView alert: {alert_data.get('symbol', 'Unknown Symbol')}")
        print(f"Alert details: Symbol={alert_data.get('symbol', 'Unknown')}, Action={alert_data.get('action', alert_data.get('signal', 'Unknown'))}")
...
@tradingview_bp.route('/alerts', methods=['GET'])
def get_tradingview_alerts():
    ...
    try:
        ...
        print(f"Fetching alerts, current count: {len(webhook_alerts)}")
        ...
        if symbol:
            ...
            print(f"Filtered alerts for symbol {symbol}: {len(filtered_alerts)} of {len(webhook_alerts)}")
        
        # Limit the number of alerts returned
        limited_alerts = filtered_alerts[-limit:] if limit > 0 else filtered_alerts
        
        print(f"Returning {len(limited_alerts)} alerts")
...
@tradingview_bp.route('/symbols/technical-data', methods=['GET'])
...
@tradingview_bp.route('/market/analysis', methods=['GET'])
...
def register_tradingview_routes(app):
    """
    Register TradingView integration routes with the Flask app
    """
    try:
        # Register the blueprint
        app.register_blueprint(tradingview_bp)
        print("Successfully registered TradingView routes through function")
        
        # Add a direct route for easier testing
        @app.route('/api/tradingview-test', methods=['GET'])
        def tradingview_direct_test():
            return jsonify({
                'success': True,
                'message': 'Direct TradingView test route is working',
                'timestamp': datetime.now().isoformat()
            })
        ...
        import traceback
        print(f"Error registering TradingView routes through function: {e}")
        traceback.print_exc()
        return False

# Alias for compatibility
register_routes = register_tradingview_routes 